...
<script>
  ...
  onMount(async () => {
    ...
    const plotData = Object.keys(propertyTypeCounts).map(propertyType => ({
      'property-type': propertyType,
      'count': propertyTypeCounts[propertyType]
    }));

    // Now, use plotData to create the plot
    plot = PlotLibrary.plot({
      marks: [
        PlotLibrary.barY(plotData, {
          x: "property-type",
          y: "count",
          fill: "steelblue"
        })
...
{#if fetchedData}
  Display the Plot component and pass the plot object
...
<style>
  /* Add any custom styling for the plot container */
  .plot-container {
    /* max-width: 900px;
    margin: 0 auto; */
    width: 100%;
